...
def get_public_menu(slug: str, db: Annotated[Session, Depends(get_db)]):
    ...
    categories = db.scalars(
        select(Category).where(Category.restaurant_id == restaurant.id).order_by(Category.sort_order, Category.id)
    ).all()
    ...
    for category in categories:
        products = db.scalars(
            select(Product)
            .where(Product.category_id == category.id, Product.available.is_(True))
